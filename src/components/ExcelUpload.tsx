...
// src/components/ExcelUpload.tsx - COMPLETE FIXED FILE
...
interface ExcelRow {
  'Tile Name': string;
  'Surface Name': string;      // ✅ Excel column name (user-friendly)
  'Material Name': string;     // ✅ Excel column name (user-friendly)
  'Category': string;
  'Tile URL': string;
  'Size': string;
  'Code': string;
...
interface ParsedTileData {
  name: string;
  tileSurface: string;         // ✅ FIXED - Matches Tile interface
  tileMaterial: string;        // ✅ FIXED - Matches Tile interface
  category: string;
  imageUrl: string;
  size: string;
  tileCode: string;
...
export const ExcelUpload: React.FC<ExcelUploadProps> = ({ onUploadComplete }) => {
  ...
  const handleDownloadExcel = () => {
    ...
    try {
      ...
      const excelData: ExcelRow[] = uploadedImages.map((img, index) => ({
        'Tile Name': '',
        'Surface Name': '',      // ✅ User-friendly column name
        'Material Name': '',     // ✅ User-friendly column name
        'Category': '',
        'Tile URL': img.url,
        'Size': '',
        'Code': generateTileCode(index),
  ...
  // ==========================================
  // ✅ STEP 2: FIXED EXCEL PROCESSING HANDLERS
  // ==========================================
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const parsedData: ParsedTileData[] = jsonData.map((row, index) => ({
            name: String(row['Tile Name'] || '').trim(),
            tileSurface: String(row['Surface Name'] || '').trim(),    // ✅ FIXED - Maps to tileSurface
            tileMaterial: String(row['Material Name'] || '').trim(),  // ✅ FIXED - Maps to tileMaterial
            category: String(row['Category'] || 'floor').trim().toLowerCase(),
            imageUrl: String(row['Tile URL'] || '').trim(),
            size: String(row['Size'] || '').trim(),
            tileCode: String(row['Code'] || '').trim(),
  ...
  const validateTileData = (tilesData: ParsedTileData[]) => {
    ...
    tilesData.forEach((tile, index) => {
      ...
      if (!tile.name) rowErrors.push('Tile Name required');
      if (!tile.tileSurface) rowErrors.push('Surface Name required');     // ✅ FIXED
      if (!tile.tileMaterial) rowErrors.push('Material Name required');   // ✅ FIXED
      if (!tile.category) rowErrors.push('Category required');
      if (tile.category && !validCategories.includes(tile.category)) {
        rowErrors.push(`Category must be: ${validCategories.join(', ')}`);
      }
  ...
  const processExcelUpload = async (tilesData: ParsedTileData[]): Promise<ProcessResults> => {
    ...
    for (let i = 0; i < totalTiles; i++) {
      ...
      try {
        ...
        // ✅ FIXED - Using correct field names that match Tile interface
        const tile = {
          name: tileData.name,
          tileSurface: tileData.tileSurface,      // ✅ FIXED - Correct field name
          tileMaterial: tileData.tileMaterial,    // ✅ FIXED - Correct field name
          category: tileData.category as 'floor' | 'wall' | 'bathroom' | 'kitchen',
          size: tileData.size,
          price: tileData.price,
          stock: tileData.stock,
