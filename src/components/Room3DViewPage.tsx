...
// const [showWallUploadModal, setShowWallUploadModal] = useState(false);
...
//   const [expandedSection, setExpandedSection] = useState<'floor' | 'wall' | 'dimensions' | 'calculator' | 'borders' | null>('floor');
...
//                         {/* Highlighter Grid */}
//                         {borderInfo.wall.highlighter.total > 0 && (
//                           <div>
...
//                         <Info className="w-2.5 h-2.5 flex-shrink-0 mt-0.5" />
//                         <span>
//                           <strong>Smart Border System:</strong><br/>
//                           {borderInfo.wall?.highlighter.total > 0 ? (
//                             <>
//                               🟢 <strong className="text-green-300">Active:</strong> Highlighter grid (30×30cm)<br/>
//                               Pattern tiles are applied on wall
//                             </>
//                           ) : borderInfo.wall?.base.total > 0 ? (
//                             <>
//                               🔴 <strong className="text-red-300">Active:</strong> Base tile grid ({wallTile.size?.width}×{wallTile.size?.height}cm)<br/>
//                               Normal wall tiles without pattern
//                             </>
//                           ) : (
//                             <>No wall tiles applied</>
//                           )}
//                         </span>
//                       </p>
//                     </div>
...
import {
  ...
  ChevronDown, ChevronUp, Eye,
  Calculator, Layers, TrendingUp,
  Grid3x3, Settings, Shuffle, Highlighter, Upload, QrCode
} from 'lucide-react';
import { Enhanced3DViewer } from '../components/Enhanced3DViewer';
import { QRScanner } from '../components/QRScanner';
import { RoomDimensionModal } from '../components/RoomDimensionModal';
...
export const Room3DViewPage: React.FC = () => {
  ...
  const [showMobileControls, setShowMobileControls] = useState(true);
  const [isMobile, setIsMobile] = useState(false);
  const [expandedSection, setExpandedSection] = useState<'floor' | 'wall' | 'dimensions' | 'calculator' | 'borders' | 'actions' | null>('floor');
  const [showDimensionEditor, setShowDimensionEditor] = useState(false);
  ...
  const toggleSection = useCallback((section: 'floor' | 'wall' | 'dimensions' | 'calculator' | 'borders' | 'actions') => {
    setExpandedSection(expandedSection === section ? null : section);
  }, [expandedSection]);
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 overflow-hidden">
              ...
              <div className={`space-y-1.5 ${isMobile && expandedSection !== 'actions' ? 'hidden' : 'block'}`}>
                ...
                {viewMode === 'view' && (
                  <div className="space-y-1.5">
                    ...
                    {/* 2️⃣ Scan Wall Tile Button (Kitchen/Bathroom only) */}
                    {isKitchenOrBathroom && (
                      <button
                        onClick={() => {
                          setShowWallScanner(true);
                          if (isMobile) setExpandedSection(null);
                        }}
                        disabled={!floorTile.sellerId}
                        className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-3 py-2.5 rounded-lg shadow-lg transition-all flex items-center justify-center gap-2 text-[10px] font-semibold hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                      >
                    ...
                    <div className="bg-blue-900/30 rounded-lg p-1.5 border border-blue-500/30">
                      <p className="text-blue-200 text-[8px] flex items-start gap-1 leading-tight">
                        <Info className="w-2.5 h-2.5 flex-shrink-0 mt-0.5" />
                        <span>
                          <strong>View Mode:</strong> Visualize tiles in 3D.
                          {!floorTile.sellerId && <><br/>📍 Scan floor tile first!</>}
                        </span>
                      </p>
                    </div>
                ...
                {viewMode === 'highlighter' && isKitchenOrBathroom && (
                  <div className="space-y-1.5">
                    ...
                    {/* 4️⃣ Shuffle Pattern Button (only if highlighter tiles exist) */}
      ...
      {showFloorUploadModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-800">Change Floor Tile</h3>
              <button 
                onClick={() => setShowFloorUploadModal(false)} 
            ...
            <p className="text-sm text-gray-600 mb-4">
              Upload a custom floor tile image or scan QR code.
            </p>
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => {
                  ...
                  setSuccess('✅ Floor tile upload feature - implement your upload logic here');
                }}
                className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-4 py-3 rounded-lg hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
              ...
              <button
                onClick={() => {
                  setShowFloorUploadModal(false);
                  setSuccess('✅ Floor QR scanner - implement your QR logic here');
                }}
                className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-3 rounded-lg hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
            ...
            <div className="mt-4 bg-blue-50 rounded-lg p-3 text-xs text-blue-800">
              <p><strong>💡 Tip:</strong> Upload high-quality images for best 3D visualization.</p>
            </div>
          </div>
        </div>
      )}
