...
// // 📍 Location: Room3DViewPage.tsx (around line 150)

// // const getWallDimensions = useCallback((wall: WallType, customWallHeight?: number) => {
// //   // ✅ CRITICAL: Use calculationDims, NOT ROOM_CONFIGS
...
// //   console.log(`   ✅ [Room3DViewPage] ${wall}: ${cols}×${rows} = ${cols * rows} tiles`);
  
// //   return { cols, rows };
  
// // }, [calculationDims, roomType, wallTile.size]); // ✅ CHANGED: Added calculationDims dependency


// // 📍 REPLACE YOUR getWallDimensions FUNCTION WITH THIS:

// const getWallDimensions = useCallback((wall: WallType, customWallHeight?: number) => {
//   // ✅ CRITICAL: Use calculationDims from state
//   const dims = calculationDims || getCalculationDimensions(roomType || 'kitchen');
...
//   <div className="flex items-center justify-between mb-1.5">
//     <div>
//       <p className="text-white text-[9px] font-semibold">Full Wall (11 Feet)</p>
//       <p className="text-gray-400 text-[7px]">Complete wall coverage</p>
//     </div>
//     <span className="bg-purple-600 text-white px-2 py-1 rounded-full text-[10px] font-bold">
...
//         Base Tiles
//       </p>
//       <p className="text-blue-100 text-[11px] font-bold">{highlighterBreakdown.wall_11ft_base}</p>
//     </div>
//     <div className="bg-orange-900/40 rounded px-1.5 py-1 border border-orange-500/30">
//       <p className="text-orange-300 text-[8px] font-semibold flex items-center gap-0.5">
//         <TrendingUp className="w-2 h-2" />
//         Highlighter
//       </p>
//       <p className="text-orange-100 text-[11px] font-bold">{highlighterBreakdown.wall_11ft_highlighter}</p>
//     </div>
//   </div>
  
//   {highlighterBreakdown.wall_11ft_highlighter > 0 && (
//     <div className="mt-1 bg-green-900/30 rounded px-1.5 py-1">
//       <p className="text-green-300 text-[7px]">
//         ✅ {((highlighterBreakdown.wall_11ft_highlighter / (highlighterBreakdown.wall_11ft_base + highlighterBreakdown.wall_11ft_highlighter)) * 100).toFixed(1)}% highlighter coverage
//       </p>
//     </div>
//   )}
// </div>
...
//   wall: { rows: number; cols: number; total: number } | null;
// }

// 📍 UPDATE BorderInfo interface

interface BorderInfo {
  floor: { rows: number; cols: number; total: number } | null;
  wall: { 
    base: { rows: number; cols: number; total: number };  // ✅ Base tile grid
    highlighter: { rows: number; cols: number; total: number };  // ✅ Highlighter grid
  } | null;
}
// ═══════════════════════════════════════════════════════════════
// CONSTANTS
...
export const Room3DViewPage: React.FC = () => {
  ...
  const [showMobileControls, setShowMobileControls] = useState(true);
  const [isMobile, setIsMobile] = useState(false);
  const [expandedSection, setExpandedSection] = useState<'floor' | 'wall' | 'dimensions' | 'calculator' | 'borders' | null>('floor'); // ✅ UPDATED
  const [showDimensionEditor, setShowDimensionEditor] = useState(false);
  ...
  // ✅ UPDATED - Added 'borders'
  const toggleSection = useCallback((section: 'floor' | 'wall' | 'dimensions' | 'calculator' | 'borders') => {
    setExpandedSection(expandedSection === section ? null : section);
  }, [expandedSection]);
  ...
  // ✅ NEW EFFECT - Calculate Border Info
  // useEffect(() => {
  ...
  //   // Calculate wall grid (for 11ft height)
  //   const isKitchenOrBathroom = roomType === 'kitchen' || roomType === 'bathroom';
  //   if (wallTile.size && calculationDims && isKitchenOrBathroom) {
  //     const dims = getWallDimensions('back', 11);
      
  //     setBorderInfo(prev => ({
  //       ...prev,
  //       wall: { rows: dims.rows, cols: dims.cols, total: dims.rows * dims.cols }
  //     }));

  //     console.log('📊 Wall border grid:', dims);
  //   }

  // }, [highlightTileBorders, floorTile.size, wallTile.size, calculationDims, roomType, getWallDimensions]);
// 📍 Location: Enhanced3DViewer effects (~3850-3950)
// ✅ PRODUCTION READY: Dual Grid Calculation
...
useEffect(() => {
  ...
  // ✅ FLOOR GRID CALCULATION (Same as before)
  if (floorTile.size && calculationDims) {
    const tileSizeM = {
      width: floorTile.size.width / 100,
      height: floorTile.size.height / 100
  ...
  // ✅ DUAL WALL GRID CALCULATION (NEW)
  const isKitchenOrBathroom = roomType === 'kitchen' || roomType === 'bathroom';
  if (wallTile.size && calculationDims && isKitchenOrBathroom) {
    // BASE GRID: Wall tile size
    const baseDims = getWallDimensions('back', 11);
    
    // HIGHLIGHTER GRID: 30×30cm fixed
    const roomMeters = {
    ...
    const HIGHLIGHTER_TILE_SIZE = 0.30; // 30cm = 0.30m
    const wallWidth = roomMeters.width; // For back wall
    ...
    console.log('📊 Wall border grids:', {
      base: `${baseDims.cols}×${baseDims.rows} (${wallTile.size.width}×${wallTile.size.height}cm)`,
      highlighter: `${highlighterCols}×${highlighterRows} (30×30cm)`
    });
  }
          ...
          <div className="p-2.5 sm:p-3 space-y-2.5">
            ...
            <section className="bg-gradient-to-br from-orange-900/50 to-red-900/50 rounded-lg p-2 border-2 border-orange-500/40">
              ...
              <div className={`space-y-1.5 ${isMobile && expandedSection !== 'borders' ? 'hidden' : 'block'}`}>
                ...
                {/* Border Info Display */}
                {highlightTileBorders && borderInfo.floor && (
                  <div className="bg-gray-800/80 rounded-lg p-2 border border-orange-500/30 space-y-1.5">
                    
                    {/* Floor Grid Info */}
                    <div className="bg-blue-900/30 rounded p-1.5 border border-blue-500/30">
                      <p className="text-blue-300 text-[9px] font-semibold mb-1 flex items-center gap-1">
                        <Package className="w-2.5 h-2.5" />
                        🟫 Floor Grid
                      </p>
                      <div className="grid grid-cols-3 gap-1 text-[8px]">
                        <div className="bg-gray-900/50 rounded px-1 py-0.5">
                          <p className="text-gray-400">Columns</p>
                          <p className="text-white font-bold">{borderInfo.floor.cols}</p>
                        </div>
                        <div className="bg-gray-900/50 rounded px-1 py-0.5">
                          <p className="text-gray-400">Rows</p>
                          <p className="text-white font-bold">{borderInfo.floor.rows}</p>
                        </div>
                        <div className="bg-blue-600/30 rounded px-1 py-0.5">
                          <p className="text-blue-300">Total</p>
                          <p className="text-white font-bold">{borderInfo.floor.total}</p>
                        </div>
                      </div>
                    </div>

                    {/* Wall Grid Info */}
                    {/* {borderInfo.wall && isKitchenOrBathroom && wallTile.texture && (
                      <div className="bg-purple-900/30 rounded p-1.5 border border-purple-500/30">
                        <p className="text-purple-300 text-[9px] font-semibold mb-1 flex items-center gap-1">
                        ...
                        <div className="grid grid-cols-3 gap-1 text-[8px]">
                          <div className="bg-gray-900/50 rounded px-1 py-0.5">
                            <p className="text-gray-400">Columns</p>
                            <p className="text-white font-bold">{borderInfo.wall.cols}</p>
                          </div>
                          <div className="bg-gray-900/50 rounded px-1 py-0.5">
                            <p className="text-gray-400">Rows</p>
                            <p className="text-white font-bold">{borderInfo.wall.rows}</p>
                          </div>
                          <div className="bg-purple-600/30 rounded px-1 py-0.5">
                            <p className="text-purple-300">Total</p>
                            <p className="text-white font-bold">{borderInfo.wall.total}</p>
                          </div>
                        </div>
                      </div>
                    )} */}

// 📍 Location: Border Tiles Section - Sidebar (~4500-4650)
// ✅ PRODUCTION READY: Dual Grid Display

{/* ✅ DUAL WALL GRID INFO */}
{borderInfo.wall && isKitchenOrBathroom && wallTile.texture && (
  <div className="bg-purple-900/30 rounded p-1.5 border border-purple-500/30 space-y-1.5">
    
    {/* Base Tiles Grid */}
    <div>
      <p className="text-red-300 text-[9px] font-semibold mb-1 flex items-center gap-1">
        <Layers className="w-2.5 h-2.5" />
        🔴 Base Wall Grid ({wallTile.size.width}×{wallTile.size.height}cm)
      </p>
      <div className="grid grid-cols-3 gap-1 text-[8px]">
        <div className="bg-gray-900/50 rounded px-1 py-0.5">
          <p className="text-gray-400">Columns</p>
          <p className="text-white font-bold">{borderInfo.wall.base.cols}</p>
        </div>
        <div className="bg-gray-900/50 rounded px-1 py-0.5">
          <p className="text-gray-400">Rows</p>
          <p className="text-white font-bold">{borderInfo.wall.base.rows}</p>
        </div>
        <div className="bg-red-600/30 rounded px-1 py-0.5">
          <p className="text-red-300">Total</p>
          <p className="text-white font-bold">{borderInfo.wall.base.total}</p>
        </div>
      </div>
    </div>

    {/* Highlighter Tiles Grid */}
    <div>
      <p className="text-green-300 text-[9px] font-semibold mb-1 flex items-center gap-1">
        <TrendingUp className="w-2.5 h-2.5" />
        🟢 Highlighter Grid (30×30cm)
      </p>
      <div className="grid grid-cols-3 gap-1 text-[8px]">
        <div className="bg-gray-900/50 rounded px-1 py-0.5">
          <p className="text-gray-400">Columns</p>
          <p className="text-white font-bold">{borderInfo.wall.highlighter.cols}</p>
        </div>
        <div className="bg-gray-900/50 rounded px-1 py-0.5">
          <p className="text-gray-400">Rows</p>
          <p className="text-white font-bold">{borderInfo.wall.highlighter.rows}</p>
        </div>
        <div className="bg-green-600/30 rounded px-1 py-0.5">
          <p className="text-green-300">Total</p>
          <p className="text-white font-bold">{borderInfo.wall.highlighter.total}</p>
        </div>
      </div>
    </div>
...
<div className="bg-orange-900/30 rounded p-1.5 border border-orange-500/30">
  <p className="text-orange-200 text-[8px] flex items-start gap-1">
    <Info className="w-2.5 h-2.5 flex-shrink-0 mt-0.5" />
    <span>
      <strong>Dual Border System:</strong><br/>
      🔴 <strong className="text-red-300">Red lines</strong> = Base wall tiles ({wallTile.size?.width}×{wallTile.size?.height}cm)<br/>
      🟢 <strong className="text-green-300">Green lines</strong> = Highlighter tiles (30×30cm)<br/>
      Both grids show simultaneously!
    </span>
  </p>
</div>
                    ...
                    <div className="bg-orange-900/30 rounded p-1.5 border border-orange-500/30">
                      <p className="text-orange-200 text-[8px] flex items-start gap-1">
                        <Info className="w-2.5 h-2.5 flex-shrink-0 mt-0.5" />
                        <span>
                          <strong>Border Feature:</strong> Black lines show exact tile placement.
                          Works with highlighter tiles too!
                        </span>
                      </p>
                    </div>
                ...
                {/* Instructions */}
                <div className="bg-gray-900/50 rounded p-1.5 border border-gray-700">
                  <p className="text-gray-300 text-[8px] leading-relaxed">
                    📐 <strong>Border lines show:</strong>
                    <br />• Exact tile grid layout
                    <br />• Row & column count
                    <br />• Total tiles visible
                    <br />• Works with highlighter!
                  </p>
                </div>
                ...
                <div className={`space-y-1.5 ${isMobile && expandedSection !== 'calculator' ? 'hidden' : 'block'}`}>
                  ...
                  <div className="bg-gray-900/70 rounded-lg p-2 border border-green-500/30 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="text-green-300 text-[9px] font-semibold">
                        📏 Room: {calculationDims.width}×{calculationDims.depth}×{calculationDims.height}ft
                      </p>
                  ...
                  {isKitchenOrBathroom && wallTile.texture && wallTile.size && highlighterBreakdown && (
                    <div className="bg-gray-800/80 rounded-lg p-2 border border-purple-500/30">
                      ...
                      <div className="text-[8px] text-gray-400 space-y-0.5 bg-gray-900/30 rounded p-1.5 mt-1.5">
                        <p className="font-semibold text-purple-300">✅ Wall Tile:</p>
                        <p>📏 {wallTile.size.width} × {wallTile.size.height} cm</p>
                        <p className="text-blue-300">
                          🏠 Perimeter: {tileCount.perimeter.toFixed(1)} ft ({calculationDims.width}×{calculationDims.depth} room)
