...
const QrIcon = ({ className = "" }) => (
  <svg className={`w-8 h-8 md:w-10 md:h-10 ${className}`} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <rect x="4" y="4" width="6" height="6" rx="1.5" />
    <rect x="14" y="4" width="6" height="6" rx="1.5" />
    <rect x="4" y="14" width="6" height="6" rx="1.5" />
    <path d="M7 7h.01M17 7h.01M7 17h.01" strokeWidth="2.5" />
...
// ============================================================================
// FEATURE CARD COMPONENT - COMPACT VERSION
// ============================================================================
...
const FeatureCard: React.FC<FeatureCardProps> = React.memo(({ title, description, icon }) => {
  return (
    <div className="relative w-full h-full bg-white rounded-[24px] p-6 md:p-8 border border-[#e8eaef] shadow-[0_8px_30px_-10px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col items-start text-left gap-5 transition-all duration-300 hover:shadow-[0_12px_40px_-10px_rgba(0,0,0,0.1)]">
      
      {/* Decorative Blob - Smaller */}
      <div className="absolute -top-12 -right-12 w-[200px] h-[200px] md:w-[250px] md:h-[250px] bg-gradient-to-br from-[#ebf0ff] to-[#f4e8ff] rounded-full pointer-events-none opacity-60" />

      {/* Icon Container - Compact */}
      <div className="relative z-10 w-[72px] h-[72px] md:w-[80px] md:h-[80px] rounded-[20px] bg-gradient-to-br from-[#3b5cfb] to-[#8d2dee] flex items-center justify-center shadow-[0_8px_20px_-6px_rgba(141,45,238,0.35)] shrink-0">
        {icon}
      </div>

      {/* Text Content - Compact */}
      <div className="relative z-10 flex flex-col gap-3">
        <h3 className="text-[22px] md:text-[26px] lg:text-[28px] font-bold text-[#1a1a1a] tracking-tight leading-[1.2]">
          {title}
        </h3>
        <p className="text-[15px] md:text-[16px] text-[#595959] leading-[1.6]">
          {description}
        </p>
      </div>
    </div>
...
export default function App() {
  const features = [
    {
      id: 'qr-integration',
      title: 'Seller Uploads Tile',
      description: 'Upload tile images, set dimensions and finish type from your seller dashboard in seconds.',
      icon: <QrIcon />
    },
    {
      id: 'ai-room-scan',
      title: 'QR Auto Genrated',
      description: 'Each tile gets a unique QR code instantly. Share it digitally or print for showroom display.',
       icon: <QrIcon />
    },

        {
      id: 'Customer Sees in 3D ID',
      title: 'Customer Sees in 3D',
      description: 'Customer instantly previews the tile applied to floor, walls, or highlight panels in a real room.',
      icon: <EyeIcon />
    },
        {
      id: 'ar-preview',
      title: 'Worker Scan QR',
      description: 'Showroom staff scan the QR code on any phone — no app install required.',
        icon: <QrIcon />
    }
  ];

  return (
    <section className="w-full bg-[#f9fafb] flex flex-col items-center justify-center py-16 lg:py-24 font-['Inter',_sans-serif] antialiased">
      
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-6 lg:px-8">
        
        {/* Header Section */}
        <div className="text-center mb-12 lg:mb-16">
          <h2 className="text-[#1a1a1a] text-3xl md:text-4xl lg:text-5xl font-extrabold mb-4 tracking-tight leading-[1.1]">
            Magic in Simple Steps
          </h2>
          <p className="text-[#595959] text-[16px] md:text-[18px] lg:text-[20px] max-w-3xl mx-auto leading-relaxed">
            Our proprietary AI bridges the gap between digital selection and physical reality.
          </p>
        </div>
        ...
        <div className="relative w-full max-w-7xl mx-auto">
          <Swiper 
            ...
            className="mySwiper !pb-16 !pt-2 !px-10 md:!px-12"
            style={{
              '--swiper-navigation-color': '#1a1a1a',
              '--swiper-navigation-size': '22px',
              '--swiper-navigation-sides-offset': '0px',
              '--swiper-pagination-color': '#3b5cfb',
