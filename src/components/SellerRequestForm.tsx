...
// ═══════════════════════════════════════════════════════════════════════════
// ✅ DEFAULT FORM STATE — single source of truth (prevents undefined fields)
// ═══════════════════════════════════════════════════════════════════════════
...
export const SellerRequestForm: React.FC = () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // FIX: Safe localStorage load — MERGE with defaults instead of replacing.
  // Guarantees every field always exists (never undefined).
  // ─────────────────────────────────────────────────────────────────────────
  const [formData, setFormData] = useState<FormData>(() => {
    try {
      const saved = localStorage.getItem('sellerRequestDraft');
      ...
      // corrupted draft -> ignore and fall back to defaults
    }
  ...
  const validateEmailDomain = (email: string): { isValid: boolean; suggestion?: string; hasTypo: boolean } => {
    const commonTypos: Record<string, string> = {
      'gmali.com': 'gmail.com', 'gmai.com': 'gmail.com', 'gmil.com': 'gmail.com',
      'gmaill.com': 'gmail.com', 'gamil.com': 'gmail.com', 'gmial.com': 'gmail.com',
      'gmaol.com': 'gmail.com', 'gnail.com': 'gmail.com',
      'yahooo.com': 'yahoo.com', 'yaho.com': 'yahoo.com', 'yahho.com': 'yahoo.com',
      'yahhoo.com': 'yahoo.com',
      'hotmial.com': 'hotmail.com', 'hotmai.com': 'hotmail.com', 'hotmali.com': 'hotmail.com',
  ...
  const checkBusinessNameExists = async (name: string): Promise<boolean> => {
    ...
    try {
      ...
      console.error('Business name check error:', err);
      setBusinessNameCheckMessage('');
      setBusinessNameExists(false);
      setCheckingBusinessName(false);
      return false;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // FIX: Sanitize payload before sending to Firestore.
  // Every field is guaranteed to be a string or explicit null — NEVER undefined.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const buildSanitizedPayload = () => {
    return {
      ...
      phone: (formData.phone || '').replace(/\D/g, ''),
      businessAddress: (formData.businessAddress || '').trim(),
      additionalInfo: formData.additionalInfo?.trim() ? formData.additionalInfo.trim() : null,
      status: 'pending',
      requestedAt: new Date().toISOString(),
      adminNotes: null,
      reviewedAt: null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'sellerRequests'), payload);
