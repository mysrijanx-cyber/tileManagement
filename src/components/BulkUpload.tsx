...
"Polished Concrete",floor,"80x80 cm",2800,30,"https://images.unsplash.com/photo-1556912172-45b7abe8b7e1?w=400","https://images.unsplash.com/photo-1556912172-45b7abe8b7e1?w=400","PC005"`;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Instructions Section */}
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6">
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
            <FileText className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-purple-600 mt-0.5 sm:mt-1 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2 gap-2">
                <h3 className="text-base sm:text-lg md:text-xl font-bold text-purple-800 truncate">CSV Bulk Upload with QR Generation</h3>
                <button
                  onClick={() => setShowInstructions(!showInstructions)}
                  className="text-purple-600 hover:text-purple-800 text-xs sm:text-sm font-medium flex-shrink-0"
                >
                  {showInstructions ? 'Hide' : 'Show'} Instructions
                </button>
              </div>
              
              <p className="text-purple-700 mb-3 sm:mb-4 text-xs sm:text-sm">
                Upload multiple tiles at once using CSV format. QR codes will be automatically generated for each tile.
              </p>
              ...
              {showInstructions && (
                <>
                  ...
                  <div className="mb-3 sm:mb-4">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-1.5 sm:gap-2 text-xs">
                      {commonTileSizes.slice(0, 12).map((size) => (
                        <span key={size} className="bg-purple-100 text-purple-800 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-center truncate">
                          {size}
                        </span>
                      ))}
                    </div>
                  ...
                  {/* Category Options */}
                  <div className="mb-3 sm:mb-4">
                    <p className="text-purple-700 text-xs sm:text-sm font-medium mb-2">Valid Categories:</p>
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 text-xs">
                      <span className="bg-purple-100 text-purple-800 px-2 sm:px-3 py-0.5 sm:py-1 rounded font-medium">floor</span>
                      <span className="bg-purple-100 text-purple-800 px-2 sm:px-3 py-0.5 sm:py-1 rounded font-medium">wall</span>
                      <span className="bg-purple-100 text-purple-800 px-2 sm:px-3 py-0.5 sm:py-1 rounded font-medium">both</span>
                    </div>
                  </div>
                </>
              )}
              ...
              <button
                onClick={downloadSampleCSV}
                className="flex items-center gap-1.5 sm:gap-2 bg-purple-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:bg-purple-700 transition-colors shadow-sm text-xs sm:text-sm"
              >
                <Download className="w-3 h-3 sm:w-4 sm:h-4" />
                <span className="hidden xs:inline">Download Sample</span>
      ...
      {/* Upload Progress */}
      {isUploading && (
        <div className="bg-white border border-purple-200 rounded-lg p-3 sm:p-4 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs sm:text-sm font-medium text-gray-700 flex items-center gap-1.5 sm:gap-2 truncate">
              <Loader className="w-3 h-3 sm:w-4 sm:h-4 animate-spin text-purple-600 flex-shrink-0" />
              <span className="hidden sm:inline">Processing bulk upload...</span>
              <span className="sm:hidden">Processing...</span>
            </span>
            <span className="text-xs sm:text-sm font-bold text-purple-600 flex-shrink-0 ml-2">{uploadProgress}%</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {/* File Upload */}
        <div className="border-2 border-dashed border-gray-300 rounded-lg sm:rounded-xl p-4 sm:p-6 md:p-8 text-center hover:border-purple-400 hover:bg-purple-50 transition-all">
          <FileText className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-gray-400 mx-auto mb-3 sm:mb-4" />
          <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-1 sm:mb-2">Upload CSV File</h3>
          <p className="text-gray-600 mb-3 sm:mb-4 text-xs sm:text-sm px-2">
            Select a CSV file from your computer
          </p>
          ...
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={isUploading}
            className="flex items-center gap-1.5 sm:gap-2 bg-purple-600 text-white px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors mx-auto shadow-sm text-xs sm:text-sm"
          >
        ...
        {/* Manual Input */}
        <div className="border border-gray-300 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6">
          <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-3 sm:mb-4">Or Paste CSV Data</h3>
          <textarea
            value={csvData}
            onChange={(e) => setCsvData(e.target.value)}
            placeholder="Paste your CSV data here...&#10;&#10;name,category,size,price,stock,imageUrl&#10;&quot;Tile Name&quot;,floor,&quot;60x60 cm&quot;,2500,100,&quot;https://...&quot;"
            disabled={isUploading}
            className="w-full h-32 sm:h-40 px-2 sm:px-3 py-1.5 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 disabled:bg-gray-100 disabled:cursor-not-allowed font-mono text-xs sm:text-sm resize-none"
            aria-label="CSV data input"
          />
          {csvData && (
            <p className="text-xs text-gray-600 mt-2">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 flex-wrap">
        <button
          onClick={handleBulkUpload}
          disabled={!csvData.trim() || isUploading || !currentUser}
          className="flex items-center justify-center gap-1.5 sm:gap-2 bg-purple-600 text-white px-4 sm:px-6 md:px-8 py-2.5 sm:py-3 rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md font-medium text-xs sm:text-sm md:text-base"
        >
          {isUploading ? (
            <>
            ...
            <>
              ...
              <span className="hidden sm:inline">Upload Tiles with QR Codes</span>
              <span className="sm:hidden">Upload with QR</span>
            </>
          )}
        </button>
        ...
        {csvData && !isUploading && (
          <button
            onClick={handleClearData}
            className="flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2.5 sm:py-3 border-2 border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-xs sm:text-sm md:text-base"
          >
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6">
        <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
          ...
          <span className="truncate">CSV Upload Tips & Best Practices</span>
        </h4>
