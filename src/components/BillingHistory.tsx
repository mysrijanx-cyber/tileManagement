...
//     if (!endDate || endDate === 'N/A') return 0; // Fixed: Prevent NaN if endDate is empty
...
//       // Fixed: Helper function to safely parse Firebase Timestamps
//       // const parseFirebaseDate = (dateVal: any): string => {
//       //   if (!dateVal) return '';
...
//       // ENHANCED HELPER: Handles both Live Firestore Timestamps and Cached Data Objects
//       const parseFirebaseDate = (dateVal: any): string => {
//         if (!dateVal) return '';
        
//         // 1. Live Firebase Timestamp (Online mode)
//         if (typeof dateVal.toDate === 'function') {
//           return dateVal.toDate().toISOString();
//         }
        
//         // 2. Cached Firebase Timestamp (Offline/Cache mode) -> YAHI BUG THA!
//         if (typeof dateVal === 'object' && dateVal !== null && 'seconds' in dateVal) {
//           // Firebase seconds ko milliseconds mein convert karna zaroori hai
//           return new Date(dateVal.seconds * 1000).toISOString();
//         }
        
//         // 3. Direct UNIX Timestamp (Number)
//         if (typeof dateVal === 'number') {
...
//         // 4. Standard String Date
//         if (typeof dateVal === 'string') {
//           // Validating if the string is actually a proper date
...
//         return ''; // Agar kuch bhi match na kare, toh empty string return karo
//       };
...
//         // Fixed: Safely parse dates before using them
...
//           ...subData,
//           end_date: safeEndDate, // pass safe date
...
//           start_date: safeStartDate, // Fixed
//           end_date: safeEndDate,     // Fixed
//           current_scan_count: subData.current_scan_count || 0,
//           last_payment_id: subData.last_payment_id,
//           created_at: safeCreatedAt, // Fixed
...
//           payment_date: safePaymentDate, // Fixed
//           days_remaining: daysRemaining,
...
//       // Fixed: Prevent NaN sorting by providing a 0 fallback for missing dates
//       switch (sortField) {
//         case 'purchase_date':
...
//                             {/* Fixed: Added ternary operator to avoid Invalid Date */}
...
export const BillingHistory: React.FC = () => {
  ...
  // ✅ ADD: Refs for debouncing and preventing concurrent loads
  ...
  const calculateSellerAnalytics = useCallback((sellerId: string): SellerAnalytics => {
    ...
    const activeSubscriptions = sellerSubscriptions.filter(sub => 
      (sub.status === 'active' || sub.status === 'pending') && !sub.is_expired
    ).length;
  ...
  // ✅ ENHANCED: parseFirebaseDate with proper cached timestamp handling
  const parseFirebaseDate = useCallback((dateVal: any): string => {
    if (!dateVal) return '';
    
    // Live Firebase Timestamp
    if (typeof dateVal.toDate === 'function') {
      return dateVal.toDate().toISOString();
    }
    
    // Cached Firebase Timestamp (object with seconds property)
    if (typeof dateVal === 'object' && dateVal !== null && 'seconds' in dateVal) {
      return new Date(dateVal.seconds * 1000).toISOString();
    }
    
    // Direct UNIX Timestamp (Number)
    if (typeof dateVal === 'number') {
      const ms = dateVal < 10000000000 ? dateVal * 1000 : dateVal;
      return new Date(ms).toISOString();
    }
  ...
  // ✅ ENHANCED: loadData with debouncing and force refresh support
  const loadData = useCallback(async () => {
    ...
    if (loadingRef.current) {
      console.log('⏭️ Load already in progress, queuing...');
    ...
    try {
      ...
      console.log('📊 Loading billing history...', forceRefresh ? '(FORCE REFRESH)' : '(CACHE OK)');
      ...
      // ✅ Check if data came from cache despite force refresh flag
      if (subscriptionsSnapshot.metadata.fromCache && forceRefresh) {
        console.warn('⚠️ Data from cache despite force refresh flag!');
        console.log('🔄 Retrying with network-only fetch in 2 seconds...');
      ...
      console.log(`✅ Fetched ${subscriptionsSnapshot.size} subscriptions (fromCache: ${subscriptionsSnapshot.metadata.fromCache})`);
      ...
      // ✅ Clear force refresh flag after successful fresh fetch
      if (!subscriptionsSnapshot.metadata.fromCache) {
        sessionStorage.removeItem('force_firestore_refresh');
        console.log('✅ Fresh data fetched, cleared force refresh flag');
      }
      ...
      const enrichedSubscriptions: EnrichedSubscription[] = subscriptionsSnapshot.docs.map(doc => {
        ...
        return {
          ...
          plan_validity_unit: plan.validity_unit || 'days',
          plan_scan_limit: scanLimit,
          payment_amount: payment.amount || plan.price || 0, // ✅ Fallback to plan price
          payment_id: payment.razorpay_payment_id,
          transaction_id: payment.transaction_id,
  ...
  // ✅ ENHANCED: calculateStatistics with proper filtering
  const calculateStatistics = useCallback((subs: EnrichedSubscription[]) => {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);

  // ✅ CHANGE 1: Filter for counts (not revenue)
  const activeSubs = subs.filter(s => 
    s.status !== 'completed' && s.status !== 'replaced'
  );

  const activeSubscriptions = activeSubs.filter(s => 
    (s.status === 'active' || s.status === 'pending') && !s.is_expired
  );
  
  // ✅ CHANGE 2: Calculate revenue from ALL subscriptions
  ...
  console.log('📊 Revenue Breakdown:', {
    ...
    replaced_subscriptions: subs.filter(s => s.status === 'replaced').length,
    completed_subscriptions: subs.filter(s => s.status === 'completed').length,
    total_revenue: allTimeRevenue,
    active_revenue: activeSubs.reduce((sum, s) => sum + (s.payment_amount || s.plan_price || 0), 0)
  });
  
  const stats: Statistics = {
    totalActive: activeSubscriptions.length,
    
    // ✅ FIX: Use all subscriptions for revenue
    totalRevenue: allTimeRevenue,
    
    expiringSoon: activeSubs.filter(s => s.is_expiring_soon && !s.is_expired).length,
    
    expiredThisMonth: activeSubs.filter(s => {
      if (!s.end_date) return false;
      const endDate = new Date(s.end_date);
      return endDate >= monthStart && endDate <= now && s.is_expired;
    }).length,
    
    renewedThisMonth: activeSubs.filter(s => {
      if (!s.created_at) return false;
      const createdDate = new Date(s.created_at);
  ...
  // ✅ ENHANCED: useEffect with event bus listener
  useEffect(() => {
    loadData();

    // Event bus listener for subscription updates
    const handleSubscriptionUpdate = (data: any) => {
      console.log('🔔 [EVENT] SUBSCRIPTION_UPDATED received:', data);
      console.log('🔄 Force refreshing billing history...');
      loadData();
    };
  ...
  const filteredAndSortedSubscriptions = useMemo(() => {
    ...
    if (filterStatus !== 'all') {
      if (filterStatus === 'active') {
        filtered = filtered.filter(s => (s.status === 'active' || s.status === 'pending') && !s.is_expired);
      } else if (filterStatus === 'expired') {
        filtered = filtered.filter(s => s.is_expired);
      } else if (filterStatus === 'expiring_soon') {
        filtered = filtered.filter(s => s.is_expiring_soon);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* REST OF THE UI CONTINUES... */}
      {/* (TABLE, MOBILE CARDS, PAGINATION, MODAL - keeping existing code) */}
...
console.log('✅ BillingHistory - PRODUCTION v8.0 (Real-time Ready with Event Bus)');