...
// import { Camera,Shield , X, Upload, Hash, AlertCircle,  Image as ImageIcon, CheckCircle, RotateCcw } from 'lucide-react';
...
// export const QRScanner: React.FC<QRScannerProps> = ({ onScanSuccess, onClose,currentUser }) => {
...
//   // ═══════════════════════════════════════════════════════════════
//   // ✅ OPTIMIZATION 2: OPTIMIZED VIDEO CONSTRAINTS
//   // ═══════════════════════════════════════════════════════════════
...
//       //     width: { ideal: 640, max: 1280 },  // Was 1920
//       //     height: { ideal: 480, max: 720 }   // Was 1080
//       //   }
//       // });
...
//   video: { 
//     facingMode: cameraFacing,
//     width: { ideal: 1280, max: 1920 },  // ✅ NEW - Higher resolution
//     height: { ideal: 720, max: 1080 }   // ✅ NEW - Better quality
//   }
// });
...
//   // ═══════════════════════════════════════════════════════════════
//   // ✅ OPTIMIZATION 3: FRAME SKIPPING + CANVAS OPTIMIZATION
//   // ═══════════════════════════════════════════════════════════════
//   const scanQRCode = () => {
...
//     // ✅ CANVAS OPTIMIZATION: Use smaller canvas for processing
...
//     // Draw video frame to canvas (scaled down if needed)
...
//   // ═══════════════════════════════════════════════════════════════
//   // ✅ OPTIMIZATION 4: NO COOLDOWN - INSTANT RESPONSE
//   // ═══════════════════════════════════════════════════════════════
...
//   //       // ✅ INSTANT CALLBACK - No await, no delay
...
//         console.log('✅ QRScanner: Worker authorized for QR scan');
//       }
...
//       // Update parsed with sanitized tileId
...
//                 <p className="text-blue-900 text-xs sm:text-sm font-medium mb-2">⚡ Optimized Scanner:</p>
//                 <ul className="text-blue-800 text-xs sm:text-sm space-y-1">
//                   <li>• Hold phone 6-8 inches from QR code</li>
//                   <li>• Ensure good lighting</li>
//                   <li>• Keep camera steady</li>
//                   <li>• ✅ <strong>Ultra-fast detection!</strong></li>
//                   <li>• 📱 <strong>Battery optimized</strong></li>
//                 </ul>
//               </div>
//             </div>
//           )}
...
//                   💡 <strong>Tip:</strong> Take a clear photo of the QR code and upload it here.
//                 </p>
//               </div>
//             </div>
//           )} */}
...
//         <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-6 text-center hover:border-green-500 transition-colors active:border-green-600 touch-manipulation">
//           <input
//             type="file"
//             accept="image/*"
//             onChange={handleImageUpload}
...
//             <div>
//               <p className="text-gray-700 font-medium text-sm sm:text-base">📁 Choose from Gallery</p>
//               <p className="text-gray-500 text-xs sm:text-sm mt-1">Select existing photo</p>
//             </div>
//           </label>
//         </div>
//       </>
//     ) : (
//       /* ✅ DESKTOP/IPAD: File Picker Only */
...
//           <div>
//             <p className="text-gray-700 font-medium text-sm sm:text-base">Upload QR Code Image</p>
//             <p className="text-gray-500 text-xs sm:text-sm mt-1">Click to select file</p>
//           </div>
//         </label>
//       </div>
//     )}
...
//     <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
//       <p className="text-gray-700 text-xs sm:text-sm">
//         💡 <strong>Tip:</strong> {isMobile ? 'Use gallery for saved QR images, or camera for new photos.' : 'Upload a clear photo of the QR code.'}
//       </p>
//     </div>
//   </div>
// )}
...
export const QRScanner: React.FC<QRScannerProps> = ({ onScanSuccess, onClose, currentUser }) => {
  ...
  // ═══════════════════════════════════════════════════════════════
  // QR DETECTION WITH WORKER AUTHORIZATION
  // ═══════════════════════════════════════════════════════════════
  const handleQRDetected = async (qrData: string) => {
    try {
      console.log('🔍 QR Data received:', qrData);
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🔍 QRScanner: Manual code search:', manualCode.trim());
      
      // ═══════════════════════════════════════════════════════════
      // GET TILE BY CODE (SINGLE PARAMETER)
      // ═══════════════════════════════════════════════════════════
      ...
      if (result.success && result.tile) {
        ...
        // AUTHORIZED - PROCEED
        setIsLoading(false);
        console.log('✅ QRScanner: Tile found via manual code:', result.tile.name);
        ...
        console.error('❌ QRScanner: Manual search failed:', result.error);
      ...
      console.error('❌ QRScanner: Manual search error:', err);
    ...
    >
      <div className="bg-white rounded-none sm:rounded-xl shadow-2xl w-full h-full sm:h-auto sm:max-w-lg sm:max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {currentUser?.role === 'worker' && (
          <div className="bg-blue-600/10 border-b border-blue-500/30 px-4 py-2 flex items-center gap-2 flex-shrink-0">
            <Shield className="w-4 h-4 text-blue-600 flex-shrink-0" />
            <p className="text-blue-800 text-xs sm:text-sm font-medium">
              🔒 Worker Mode: Only tiles from your showroom can be scanned
            </p>
          </div>
        )}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1">
          ...
          {scanMode === 'manual' && (
            <form onSubmit={handleManualSubmit} className="space-y-3 sm:space-y-4">
              ...
              >
                {isLoading ? 'Searching...' : 'Search Tile'}
              </button>
