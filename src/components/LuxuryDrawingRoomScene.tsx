...
// export default LuxuryDrawingRoomScene; 
// ✅ PRODUCTION READY - LUXURY DRAWING ROOM (NO CHANDELIER)
import React, { useMemo } from 'react';
import * as THREE from 'three';
...
export const LuxuryDrawingRoomScene: React.FC<LuxuryDrawingRoomSceneProps> = ({
  ...
  }, [floorTexture, floorTileSize, W, D, highlightBorders]);

  // ✅ ENHANCED CEILING WITH TRAY DESIGN & CROWN MOLDING (NO CHANDELIER)
  const EnhancedCeiling = () => (
    <group>
      {/* Main Ceiling */}
      <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, H, 0]} receiveShadow>
      ...
      {/* ✅ CENTRAL CEILING LIGHT (Replacement for Chandelier) */}
      <group position={[0, H - 0.32, 0]}>
        {/* Modern Flush Mount Fixture */}
        <mesh castShadow>
        ...
        {/* Central Point Light */}
        <pointLight position={[0, -0.15, 0]} intensity={2.2} distance={11} color="#fff8f0" />
      </group>
    </group>
  );
